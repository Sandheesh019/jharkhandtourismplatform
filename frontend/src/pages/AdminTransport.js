...
const typeIcon = { BUS:'🚌', TRAIN:'🚂', CAB:'🚕', AUTO:'🛺' };
const typeColor = { BUS:'#11998e', TRAIN:'#2193b0', CAB:'#f7971e', AUTO:'#8e2de2' };
...
export default function AdminTransport() {
  ...
  const load = () => { setLoading(true); api.get('/transport').then(r => { setData(r.data); setLoading(false); }); };
  const flash = m => { setMsg(m); setTimeout(() => setMsg(''), 3000); };

  const openAdd = () => { setEditId(null); setForm({}); setErr(''); setShowForm(true); };
  const openEdit = t => { setEditId(t.id); setForm({ type: t.type, source: t.source, destination: t.destination, operatorName: t.operatorName, contactNumber: t.contactNumber, fare: t.fare, schedule: t.schedule }); setErr(''); setShowForm(true); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const closeForm = () => { setShowForm(false); setForm({}); setEditId(null); };

  const handleSubmit = async e => {
    e.preventDefault(); setErr('');
    const payload = { ...form, fare: +form.fare };
    try {
      if (editId) { await api.put(`/transport/${editId}`, payload); flash('✅ Transport updated!'); }
      else { await api.post('/transport', payload); flash('✅ Transport added!'); }
      closeForm(); load();
    } catch (error) {
  ...
  const handleDelete = async id => { if (!window.confirm('Delete?')) return; await api.delete(`/transport/${id}`); flash('✅ Deleted.'); load(); };
  ...
  return (
    <AdminLayout active="/admin/transport" color="#8e2de2" title="🚌 Transport" count={data.length}
      ...
      {showForm && (
        <div style={s.formCard}>
          <div style={s.formHeader}>
            <h3 style={{ ...s.formTitle, color: '#8e2de2' }}>{editId ? '✏️ Edit Transport' : '➕ Add New Transport'}</h3>
          ...
          <form onSubmit={handleSubmit} style={s.grid}>
            <div style={s.fw}><label style={s.lbl}>Type</label>
              <select style={s.inp} value={form.type||''} onChange={e=>setForm({...form,type:e.target.value})} required>
                <option value="">Select</option><option>BUS</option><option>TRAIN</option><option>CAB</option><option>AUTO</option>
              </select>
            </div>
            {f('source','From (Source)')} {f('destination','To (Destination)')}
            {f('operatorName','Operator Name')} {f('contactNumber','Contact Number')}
            {f('fare','Fare (₹)','number')} {f('schedule','Schedule')}
            <button type="submit" style={{ ...s.saveBtn, background: '#8e2de2', gridColumn: '1/-1' }}>
              {editId ? '💾 Update Transport' : '✅ Save Transport'}
            </button>
          </form>
        </div>
      )}
        ...
        <div style={s.list}>
          {data.map(t => (
            <div key={t.id} style={s.row}>
              <div style={{ ...s.imgBox, background: (typeColor[t.type]||'#8e2de2')+'22' }}>
                <span style={{ fontSize: '1.8rem' }}>{typeIcon[t.type]||'🚗'}</span>
              </div>
              <div style={s.info}>
                <div style={s.rowTitle}>{t.operatorName}</div>
                <div style={s.meta}>
                  <span style={{ ...s.badge, background: (typeColor[t.type]||'#8e2de2')+'22', color: typeColor[t.type]||'#8e2de2' }}>{t.type}</span>
                  &nbsp; {t.source} → {t.destination}
                </div>
                <div style={s.meta}>💰 ₹{t.fare} &nbsp;|&nbsp; 📞 {t.contactNumber}</div>
                <div style={s.desc}>🕐 {t.schedule}</div>
              </div>
              <div style={s.btnGroup}>
                <button style={s.editBtn} onClick={() => openEdit(t)}>✏️ Edit</button>
                <button style={s.del} onClick={() => handleDelete(t.id)}>🗑️ Delete</button>
              </div>
            </div>
          ))}
          {data.length === 0 && <div style={s.empty}>📭 No transport found.</div>}
        </div>
      )}
    </AdminLayout>
  );
